...
process broker() {
	...
	while (1) {	
		...
		byte group_byte = (topic & 0xFF00) >> 8;
		byte topic_byte = topic & 0xFF;
		if(group_byte == 0x00) {
			for(; group_byte<0xFF; ) {
				group_byte += 1;
				topic16 temp_topic = (group_byte<<8) + topic_byte;
				handler_over_topic(temp_topic, data);
			}
		} else {
			handler_over_topic(topic, data);
		}
		deQ(publishQ);
	}
}
